...
function e(f,i,an)
{
        with(document)
        {
          write('<div align=right><table id=m'+i+' style="visibility:hidden"><tr><td class=ei><a name="cid'+i+'"></a><a href="#cid" onclick="copyid('+i+','+f+');">#'+i+'</a></td>');
                  ...
                  a("/sozluk.php?process=privmsg&islem=yenimsj&gkime="+an+"&gkonu="+i+" nolu entry;","/msj","mesaj at");;
...
function ors(t)
{
        document.write('<select style="width:158px" class=tedit onchange="if(this.selectedIndex>0){window.open(this.options[this.selectedIndex].value+\''+t+'\');this.selectedIndex=0;}">'+
      '<option>..bilgi kaynaklarý..</option>'+
      '<option value="http://www.google.com/search?q=">gugyl</option>'+
      ...
      '<option value="http://www.allmusic.com/cg/amg.dll?p=amg&opt1=3&sql=">sarky</option>'+
      '</optgroup>'+

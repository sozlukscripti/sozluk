...
if ($verified_user)
echo "<META HTTP-EQUIV=\"REFRESH\" CONTENT=\"300;URL=sozluk.php?process=hepsi\">";
$yil=date("Y");
$btarih = date ("d/m/Y");
?>
<meta name="keywords" content="<? echo "Tüm baþlýklar - $sitename - $description"; ?>">
<meta name="description" content="<? echo "Tüm baþlýklar - $sitename - $description"; ?>">
<title><? echo "Tüm baþlýklar - $sitename - $description"; ?></title></head>
<?
...
$sor = mysql_query("select id from konucuklar WHERE statu='' ORDER BY 'tarih' desc");
$w = mysql_num_rows($sor);
$ws=ceil($w/40);

$sor = mysql_query("select gun from mesajciklar WHERE statu=''");
...
if ($sayfa != 1) {
if (!$yesterday)
echo "<a class=but href=?process=hepsi&sayfa=$linksayfa><font face=verdana size=1><<</font></a> ";
else
echo "<a class=but href=?process=hepsi&yesterday=vlk&sayfa=$linksayfa><font face=verdana size=2><<</font></a> ";
}
}
...
if ($sayfa == $i) {
if ($yesterday)
echo " <OPTION value=sozluk.php?process=hepsi&yesterday=vlk&sayfa=$i selected>$i</OPTION>";
else
echo " <OPTION value=sozluk.php?process=hepsi&sayfa=$i selected>$i</OPTION>";
} // if
else {
if ($yesterday)
echo "<OPTION value=sozluk.php?process=hepsi&yesterday=vlk&sayfa=$i>$i</OPTION>";
else
echo "<OPTION value=sozluk.php?process=hepsi&sayfa=$i>$i</OPTION>";
} // new
...
echo " <a class=but href=?process=hepsi&yesterday=vlk&sayfa=$linksayfa><font face=verdana size=1>>></font></a>";
}
...
$cumle = "
   SELECT
      ...
      (
         SELECT count(id)
         FROM mesajciklar
         WHERE sira = konucuklar.id and statu = ''
      ) AS say
   FROM konucuklar 
   ORDER BY tarih DESC
   LIMIT $alt,$max

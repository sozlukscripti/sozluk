...
$sor = mysql_query("select id from konucuklar WHERE statu='' and `gun` = '$dunb' and `ay` = '$ay' ORDER BY 'tarih'");
$w = mysql_num_rows($sor);
$ws=ceil($w/40);

$sor = mysql_query("select gun from mesajciklar WHERE statu='' and `gun` = '$dunb' and `ay` = '$ay'");
...
rastgele birgün baþlýklarý.. ($w ba\$lik)<br>
sayfa
";
...
$listele = mysql_query("SELECT id,baslik,tarih,gun FROM konucuklar WHERE statu='' and `gun` = '$dunb' and  `ay` = '$ay' ORDER BY 'tarih' desc limit $alt,$max");

...
if ($verified_user)
echo "<META HTTP-EQUIV=\"REFRESH\" CONTENT=\"300;URL=sozluk.php?process=yesterday\">";
$yil=date("Y");
$btarih = date ("d/m/Y");

?>
<meta name="keywords" content="<? echo "Dünün baþlýklarý - $sitename - $description"; ?>">
<meta name="description" content="<? echo "Dünün baþlýklarý - $sitename - $description"; ?>">
<title><? echo "Dünün baþlýklarý - $sitename - $description"; ?></title></head>
<?
$max = 40;
if (!$_GET["sayfa"])  { $_GET["sayfa"]=1; }
$alt = ($_GET["sayfa"] - 1)  * $max;

$sor = mysql_query("select id from konucuklar WHERE statu='' and `gun` = '$dun' and `ay` = '$ay' and yil='$yil' ORDER BY 'tarih'");
$w = mysql_num_rows($sor);
$ws=ceil($w/40);

$sor = mysql_query("select gun from mesajciklar WHERE statu='' and `gun` = '$dun' and `ay` = '$ay'");
...
$cumle = "
   SELECT
      ...
      (
         SELECT count(id)
         FROM mesajciklar
         WHERE sira = konucuklar.id and statu = '' and gun='$dun' and ay='$ay' and yil='$yil'
      ) AS say
   FROM konucuklar
   WHERE gun='$dun' and ay='$ay' and yil='$yil' and statu=''
   ORDER BY tarih DESC
   LIMIT $alt,$max
";

...
mysql_query($sorgu);
$sorgu = "UPDATE mesajciklar SET `updatesebep` = 'Silinen entry tekrar baþlýða eklendi.' WHERE id='$canlandir'";
mysql_query($sorgu);
echo "$canlandir canlandirildi.";
}
else {
echo "Akýllanan Entryler";

$max = 40;
if (!$_GET["sayfa"])  { $_GET["sayfa"]=1; }
$alt = ($_GET["sayfa"] - 1)  * $max;

$sor = mysql_query("SELECT id FROM mesajciklar WHERE `statu`='akillandim'");
...
$sorgu = "SELECT id,statu FROM mesajciklar WHERE `statu`='akillandim' limit $alt,$max";
...
<OL>
  ...
  <DIV class=div2 align=right><font size=1>Patlatan: <A  href=\"sozluk.php?process=privmsg&islem=yenimsj&gkime=$yazartitle&gkonu=[MOD] $id nolu patlattýðýnýz entry\">$silen</a> | Patlama Sebebi: $silsebep</DIV>
  </li>
  </ol>
  </center>
";

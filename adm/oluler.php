...
mysql_query($sorgu);
$sorgu = "UPDATE mesajciklar SET `updatesebep` = 'Silinen entry tekrar basliga eklendi.' WHERE id='$canlandir'";
mysql_query($sorgu);
...
$sor = mysql_query("SELECT id FROM mesajciklar WHERE `statu`='silindi' ORDER BY `id` desc");
...
$sorgu = "SELECT id,statu FROM mesajciklar WHERE `statu`='silindi' ORDER BY `id` desc limit $alt,$max";
...
if (!$baslik) {
echo "<div class=dash><center><b>Bu id'e ait bir entry yok.";
...
$sil = "<a href=http://www.ankasozluk.com/sozluk.php?process=adm&islem=entrysil&id=$id><font size=1>Yuh Hayvan(SIL)</a>";

if ($ayazar == $verified_user or $verified_kat == "admin" or $verified_kat == "mod" or $verified_kat == "modust")
$duzenle = "<a href=http://www.ankasozluk.com/sozluk.php?process=eduzenle&id=$id&sr=$sira><font size=1>Düzenle ve Aktif et</a> -";
else
$duzenle = "";
...
// admin check
if ($verified_user)
$msg = "<A  href=\"http://www.ankasozluk.com/sozluk.php?process=privmsg&islem=yenimsj&gkime=$yazartitle&gkonu=$id nolu patlayan entryiniz\"><font size=1>msg</A>|</font>";
echo "
<OL>
  <LI value=$say>
  ...
  <DIV class=div2 align=right><font size=1>Patlatan: <A  href=\"sozluk.php?process=privmsg&islem=yenimsj&gkime=$yazartitle&gkonu=[mod] $id nolu patlattiðiniz entry\">$silen</a> | Patlama Sebebi: $silsebep</DIV>
  </li>
  </ol>
  </center>
";

...
<table width=\"351\" border=\"0\">
  ...
  <tr>
    ...
    <td><select name=\"yetki\" id=\"yetki\">
	  ...
	  <option value=\"modust\" $modust>mod</option>
    </select></td>
  </tr>
  ...
  <tr>
    ...
    <td><select name=\"durum\" id=\"durum\">
      ...
      <option value=\"sus\" $sus>silik</option>
            </select></td>
  </tr>
  <tr>
    <td>Cinsiyet</td>
...
echo "
<u><a href=?process=adm&islem=kullanici&update=ok>Kullanýcý düzenle</a></u>
<table width=\"601\" border=\"1\">
  <tr>
    ...
    <td width=\"100\"><div align=\"center\"><strong>ÝP</strong></div></td>
  </tr>
  ";
$sorgu = "SELECT * FROM user ORDER BY `id` desc LIMIT 1000";
$sorgulama = @mysql_query($sorgu);
if (@mysql_num_rows($sorgulama)>0){
//kayýtlarý listele
while ($kayit=@mysql_fetch_array($sorgulama)){
...
$regip=$kayit["regip"];
echo "<tr>
    <td><a href=\"sozluk.php?process=adm&islem=kullanici&update=ok&gnick=$nick\">$nick</a></td>
    <td>$email</td>
    <td>$durum</td>
    <td>$yetki</td>
    <td>$regip</td>
